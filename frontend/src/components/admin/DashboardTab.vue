...
const LOG_COLUMNS = [
  ...
  { key: 'duration_ms', label: 'Durée (ms)', render: (r) => (r.duration_ms != null ? r.duration_ms : '—') },
];
...
const roleTotal = computed(() => {
  ...
  return Math.max(stats.value.by_role.reduce((s, r) => s + r.count, 0), 1);
});
  ...
  <div class="dashboard">
    ...
    <div class="dashboard-header">
      ...
      <div class="period-toggle">
        <button
          :class="['period-btn', { active: period === 7 }]"
          @click="period = 7"
        >7 jours</button>
        <button
          :class="['period-btn', { active: period === 30 }]"
          @click="period = 30"
        >30 jours</button>
      </div>
    </div>
...
.legend-success { background: var(--color-primary); }
.legend-error   { background: var(--color-error, #ef4444); }
